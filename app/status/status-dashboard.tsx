...
const money = (value: number) => `${value >= 0 ? "+" : "−"}€${(Math.abs(value) / 1_000_000).toFixed(1)}m`;
...
export function StatusDashboard() {
  ...
  return <>
    ...
    <section className="data-refresh-panel" id="data-refresh">
      <div className="data-refresh-intro"><div className="section-index">05 / PLAYER DATA LIFECYCLE</div><h2>Weekly, validated,<br />reviewed before release.</h2><p>Latest serving snapshot: season {data.dataRefresh.season}, generated {formatDate(data.dataRefresh.generatedAt)} from {data.dataRefresh.source.name} ({data.dataRefresh.source.license}). The newest source match is {data.dataRefresh.source.latestMatchDate ?? "not reported"}.</p><strong><i /> {data.dataRefresh.status.toUpperCase()}</strong></div>
      <div><div className="refresh-kpis"><article><span>PLAYERS</span><strong>{data.dataRefresh.counts.candidatePlayers}</strong></article><article><span>CLUBS</span><strong>{data.dataRefresh.counts.clubs}</strong></article><article><span>TRANSFERS</span><strong>{data.dataRefresh.counts.transfers}</strong></article><article><span>VALUE CHANGES</span><strong>{data.dataRefresh.counts.valueChanges}</strong></article></div><div className="refresh-gates">{data.dataRefresh.gates.map((gate) => <article key={gate.name}><i>{gate.passed ? "✓" : "!"}</i><div><strong>{gate.name}</strong><p>{gate.detail}</p></div></article>)}</div></div>
      <div className="change-ledger"><div><h3>Detected club changes</h3>{data.dataRefresh.changes.transfers.length ? data.dataRefresh.changes.transfers.slice(0, 6).map((change) => <article key={change.playerId}><strong>{change.name}</strong><span>{change.from} → {change.to}</span></article>) : <p>No club changes in the latest promoted snapshot.</p>}</div><div><h3>Largest value changes</h3>{data.dataRefresh.changes.largestValueChanges.length ? data.dataRefresh.changes.largestValueChanges.slice(0, 6).map((change) => <article key={change.playerId}><strong>{change.name}</strong><span>{money(change.changeEur)}</span></article>) : <p>No value changes in the latest promoted snapshot.</p>}</div></div>
      <p className="release-policy">{data.dataRefresh.releasePolicy}</p>
    </section>
    <section className="status-grid">
      <div><div className="section-index">06 / FIXTURE SYNC LEDGER</div><h2>Recent automation runs</h2>{data.runs.length ? <div className="run-ledger">{data.runs.map((run) => <article key={run.id}><i className={run.status} /><div><strong>{run.status === "success" ? "Fixture sync completed" : "Fixture sync failed"}</strong><span>{formatDate(run.completed_at)} · {run.trigger}</span></div><div><strong>{run.duration_ms} ms</strong><span>{run.fixtures_fetched} fetched · {run.createdCandidates ?? run.created_candidates} upcoming · {run.gradedCandidates ?? run.graded_candidates} graded</span></div></article>)}</div> : <p className="status-empty">The deployed fixture ledger is healthy; detailed run history begins with the next instrumented sync.</p>}</div>
      <aside><div className="section-index">07 / DEPLOYED ARTIFACTS</div><h2>Model versions</h2>{Object.entries(data.models).map(([name, version]) => <div className="model-version" key={name}><span>{name}</span><code>{version}</code></div>)}<p>Versioned artifacts are schema-validated before deployment. Fixture data source: {data.source}. Detailed provider errors remain restricted to the protected diagnostics endpoint.</p></aside>
    </section>
  </>;
}
