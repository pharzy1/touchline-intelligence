...
type Player = { player_id: number; name: string; club: string; position: string; age: number; appearances: number; goals: number; assists: number; minutes: number; goals_per_90: number; assists_per_90: number; market_value_eur: number };
  ...
  return (
    <section className="scout-workbench shell" aria-label="Player similarity search">
      <aside className="scout-controls">
        ...
        <div className="search-results" aria-label="Player search results">{results.slice(0, 7).map((player) => <button className={selected?.player_id === player.player_id ? "selected" : ""} type="button" key={player.player_id} onClick={() => { setSelected(player); setQuery(player.name); }}><span><strong>{player.name}</strong><small>{player.club}</small></span><em>{money(player.market_value_eur)}</em></button>)}</div>
      ...
      <div className="scout-results-panel">
        ...
        {selected && <div className="reference-card"><div><span>REFERENCE</span><h2>{selected.name}</h2><p>{selected.club} · {positionLabel(selected.position)} · Age {selected.age}</p></div><strong>{money(selected.market_value_eur)}</strong></div>}
        <div className={`matches-list ${loading ? "loading" : ""}`} aria-live="polite">
          ...
          {matches.map((player, index) => <article className="match-card" key={player.player_id}>
            <span className="match-rank">0{index + 1}</span><div className="match-person"><h3>{player.name}</h3><p>{player.club} · Age {player.age}</p><div>{player.shared_signals.map((signal) => <span key={signal}>{signal}</span>)}</div></div>
        ...
        <p className="scout-disclaimer">Similarity describes statistical profile—not quality, potential, tactical fit, or availability. Always pair model output with human scouting.</p>
      </div>
    </section>
  );
}
