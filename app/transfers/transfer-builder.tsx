...
type Player = { player_id: number; name: string; club: string; position: string; age: number; appearances: number; goals: number; assists: number; minutes: number; goals_per_90: number; assists_per_90: number; minutes_per_appearance: number; international_caps: number; market_value_eur: number };
...
export function TransferBuilder() {
  ...
  return <section className="transfer-builder shell">
    <aside className="transfer-search"><div className="control-head"><span>REFERENCE PLAYER</span><span>414 PROFILES</span></div><label className="search-field"><span>Search player or club</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="e.g. Erling Haaland" /></label><div className="search-results">{searchResults.slice(0, 7).map((player) => <button type="button" key={player.player_id} className={reference?.player_id === player.player_id ? "selected" : ""} onClick={() => chooseReference(player)}><span><strong>{player.name}</strong><small>{player.club}</small></span><em>{money(player.market_value_eur)}</em></button>)}</div><div className="transfer-help"><strong>How to use this</strong><p>Pick a reference player, then add up to three same-position alternatives. Touchline combines the scouting index with fresh valuation-model inference.</p></div></aside>
    <div className="transfer-main"><div className="result-top"><span>REPLACEMENT SHORTLIST</span><button type="button" className="copy-scenario" onClick={copy}>{copied ? "LINK COPIED" : "SHARE SCENARIO ↗"}</button></div>{reference && <div className="transfer-reference"><div><span>REFERENCE</span><h2>{reference.name}</h2><p>{reference.club} · {positionLabel(reference.position)} · age {reference.age}</p></div><div><small>MODEL COST</small><strong>{reference.estimate ? money(reference.estimate.estimateEur) : "—"}</strong><span>listed {money(reference.market_value_eur)}</span></div></div>}
      <div className="replacement-grid">{candidates.map((player) => { const selected = alternatives.some((item) => item.player_id === player.player_id); const valued = alternatives.find((item) => item.player_id === player.player_id); const saving = reference?.estimate && valued?.estimate ? reference.estimate.estimateEur - valued.estimate.estimateEur : null; return <article className={selected ? "selected" : ""} key={player.player_id}><button type="button" onClick={() => toggleAlternative(player)} aria-pressed={selected}><span>{selected ? "REMOVE" : "COMPARE"}</span><strong>{selected ? "✓" : "+"}</strong></button><h3>{player.name}</h3><p>{player.club} · age {player.age}</p><div className="replacement-metrics"><span><small>PROFILE</small><strong>{player.similarity}%</strong></span><span><small>LISTED</small><strong>{money(player.market_value_eur)}</strong></span><span><small>MODEL COST</small><strong>{valued?.estimate ? money(valued.estimate.estimateEur) : "select"}</strong></span></div>{saving !== null && <div className={saving >= 0 ? "saving positive" : "saving negative"}>{saving >= 0 ? `${money(saving)} potential saving` : `${money(Math.abs(saving))} cost premium`}</div>}<div className="replacement-signals">{player.shared_signals.map((signal) => <span key={signal}>{signal}</span>)}</div></article>; })}</div>
