...
export function TransferBuilder() {
  const [query, setQuery] = useState("Erling Haaland"); const [searchResults, setSearchResults] = useState<Player[]>([]); const [reference, setReference] = useState<PlayerWithEstimate | null>(null); const [alternatives, setAlternatives] = useState<PlayerWithEstimate[]>([]); const [candidates, setCandidates] = useState<Match[]>([]); const [error, setError] = useState(""); const [copied, setCopied] = useState(false); const [cloudMessage, setCloudMessage] = useState("");
  ...
  const saveToWorkspace = async () => {
    if (!reference) return; setCloudMessage("Saving securely…");
    const response = await fetch("/api/workspace/plans", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ kind: "transfer", name: `${reference.name} replacement shortlist`, description: `${alternatives.length} model-valued alternatives to ${reference.name}.`, payload: { url: `${window.location.pathname}${window.location.search}`, summary: { reference: reference.name, alternatives: alternatives.map((player) => player.player_id), candidate_count: alternatives.length }, referenceId: reference.player_id, alternativeIds: alternatives.map((player) => player.player_id) } }) });
    ...
    const data = await response.json(); setCloudMessage(response.ok ? "Saved to your private workspace." : data.error ?? "Cloud save failed. The scenario remains available here.");
  };
  ...
  return <section className="transfer-builder shell">
    ...
    <div className="transfer-main"><div className="result-top"><span>REPLACEMENT SHORTLIST</span><div><button type="button" className="copy-scenario" onClick={copy}>{copied ? "LINK COPIED" : "SHARE SCENARIO ↗"}</button><button type="button" className="copy-scenario workspace-save" onClick={saveToWorkspace}>SAVE TO WORKSPACE</button></div></div>{cloudMessage && <p className="cloud-save-message" role="status">{cloudMessage}</p>}{reference && <div className="transfer-reference"><PlayerAvatar name={reference.name} position={reference.position} photo={reference.photo} size="large" /><div><span>REFERENCE</span><h2>{reference.name}</h2><p>{reference.club} · {positionLabel(reference.position)} · age {reference.age}</p></div><div><small>MODEL COST</small><strong>{reference.estimate ? money(reference.estimate.estimateEur) : "—"}</strong><span>listed {money(reference.market_value_eur)}</span></div></div>}
