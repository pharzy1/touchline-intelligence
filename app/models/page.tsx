  ...
  return <main className="models-page">
    ...
    <div className="shell models-content">
      ...
      <section className="gate-panel position-model-panel">
        ...
        <div className="position-model-grid">{Object.entries(model.position_models).map(([position, positionModel]) => <article key={position}><span>{position === "Attack" ? "FORWARD" : position.toUpperCase()}</span><strong>{positionModel.metrics.cross_validation.r2_mean.toFixed(3)}</strong><small>mean CV R² · {positionModel.records} records</small><div><b>Ridge MAE</b><em>{money(positionModel.metrics.model_comparison.ridge.mae_eur)}</em></div><div><b>Tree MAE</b><em>{money(positionModel.metrics.model_comparison.gradient_boosted_trees.mae_eur)}</em></div><div><b>Serving</b><em>{positionModel.promotion.serving_choice.toUpperCase()}</em></div><p>{positionModel.promotion.reason}</p></article>)}</div>
      </section>
