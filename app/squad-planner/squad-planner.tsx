...
export function SquadPlanner({ players, modelVersion, datasetUpdatedAt, season }: { players: Player[]; modelVersion: string; datasetUpdatedAt: string; season: number }) {
  ...
  const saveToWorkspace = async () => {
    setCloudMessage("Saving securely…");
    const response = await fetch("/api/workspace/plans", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ kind: "squad", name: planName.trim() || `${club} ${formation}`, description: `${proposedSquad.length}-player squad plan with ${deals.length} proposed moves.`, payload: { url: `${window.location.pathname}${window.location.search}`, summary: { club, formation, budget_millions: Math.round(budget / 1_000_000), squad_players: proposedSquad.length, proposed_moves: deals.length }, squad: slots.map((slot) => lineup[slot.id]?.player_id ?? 0), deals: deals.map((deal) => ({ soldId: deal.sold.player_id, boughtId: deal.bought.player_id })) } }) });
    if (response.status === 401) { window.location.href = `/signin-with-chatgpt?return_to=${encodeURIComponent(`${window.location.pathname}${window.location.search}`)}`; return; }
    const data = await response.json(); setCloudMessage(response.ok ? "Saved to your private workspace." : data.error ?? "Cloud save failed. Your local plan is unchanged.");
  };
  ...
  return <section className="squad-workspace shell" aria-label="Squad planning workspace">
    ...
    <div className="window-board">
      ...
      <section className="saved-plans"><div><span className="section-index">06 / SAVED PLANS</span><h2>Name it. Reopen it.</h2><p>Keep a quick copy on this device or sign in to synchronize a private, versioned plan across sessions.</p><div className="save-plan-controls"><label><span>PLAN NAME</span><input aria-label="Plan name" value={planName} onChange={(event) => setPlanName(event.target.value)} placeholder={`${club} summer window`} /></label><button type="button" onClick={savePlan}>SAVE ON DEVICE</button><button type="button" className="cloud-save" onClick={saveToWorkspace}>SAVE TO WORKSPACE</button></div>{(saveMessage || cloudMessage) && <p className="save-message" role="status">{cloudMessage || saveMessage}</p>}</div><div className="saved-plan-list">{savedPlans.map((plan) => <article key={plan.id}><div><strong>{plan.name}</strong><span>{plan.club} · {plan.formation} · {new Date(plan.savedAt).toLocaleDateString()}</span></div><button type="button" onClick={() => openPlan(plan)}>OPEN</button><button type="button" className="delete-plan" aria-label={`Delete ${plan.name}`} onClick={() => deletePlan(plan.id)}>×</button></article>)}{!savedPlans.length && <p>No device-only plans yet. Cloud plans appear in your private Workspace.</p>}</div></section>
    </div>
